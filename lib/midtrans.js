...
// QRIS otomatis (Snap) DISEMBUNYIKAN sampai akun Midtrans production aktif:
// harus dinyalakan eksplisit dengan QRIS_OTOMATIS=true, walau key sudah terisi.
// Mati = pembeli bayar ke QRIS statis dari admin + upload bukti, admin ACC/tolak.
const qrisOtomatis = process.env.QRIS_OTOMATIS === "true";
export const midtransReady =
  qrisOtomatis && Boolean(serverKey) && !/^ISI_|^isi-/.test(serverKey);
